...
namespace YH.Virtual_ECG_Monitor
{
    public class Content_Wave
    {
        ...
        public float[,] GetWaveData_ECG(Rhythm eRhythm, int nHeartRate)
        {
            float[,] wavedata;
            //float baseAMP = 1.0f;                             
            Wave_ECG wave_ECG = new Wave_ECG(eRhythm);                   //获取指定编号ECG波形

            float[,] currentwavedata = wave_ECG.WaveData_ECG.WaveData;    //获取ECG波形的数据
            int currentwaverate = wave_ECG.WaveData_ECG.Rate;             //获取ECG波形的频率
            float baseAMP = wave_ECG.WaveData_ECG.BaseAMP;                //原始数据放大倍数
        ...
        public float[] GetWaveData_ABP(int nPlot, int nSystolic,int nDiastolic)
        {
            ...
            Wave_ABP wave_ABP = new Wave_ABP();                             //获取指定编号ABP波形

            float[] currentwavedata = wave_ABP.WaveData_ABP.WaveData;       //获取ABP波形的数据
            int currentwaverate = wave_ABP.WaveData_ABP.Rate;               //获取ABP波形的频率
            float baseAMP = wave_ABP.WaveData_ABP.BaseAMP;                  //原始数据放大倍数  
            int plotCount = currentwavedata.GetLength(0);
        ...
        public float[] GetWaveData_PLETH(int plot, int nSpo2)
        {
            float[] wavedata = new float[0];
            //const float baseAMP = 0.5f;                                        //原始数据放大倍数  
            Wave_PLETH wave_PLETH = new Wave_PLETH();                           //获取指定编号PLETH波形

            float[] currentwavedata = wave_PLETH.WaveData_PLETH.WaveData;       //获取PLETH波形的数据
            int currentwaverate = wave_PLETH.WaveData_PLETH.Rate;               //获取PLETH波形的频率
            float baseAMP = wave_PLETH.WaveData_PLETH.BaseAMP;                  //原始数据放大倍数 
            int plotCount = currentwavedata.GetLength(0);
        ...
        public float[] GetWaveData_RESP(RespType eRespType, int plot, int nRespRate, int nInspCapacity, int nRespRatio)
        {
            ...
            Wave_RESP wave_RESP = DataToObject.To<Wave_RESP>(1);//    new Wave_RESP();                           //获取指定编号PLETH波形

            float[] currentwavedata = wave_RESP.WaveData[,(int)eRespType];       //获取RESP波形的数据
            int currentwaverate = wave_RESP.WaveData_RESP.Rate;               //获取RESP波形的频率
            float baseAMP = wave_RESP.WaveData_RESP.BaseAMP;                    //原始数据放大倍数 
            ...
            {
                //预设波形数据
                leftplotCount = plotCount * wave_RESP.WaveData_RESP.Ratio / 100;                    //预存RESP波形左边点个数
